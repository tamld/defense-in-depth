...
|---------------------------|------------------|:--------------------------:|
| **1. Brainstorming / Drafts / Memory** | `.gemini/brain/`, `.claude/`, `.cursor/`, or `/tmp/` | ❌ Gitignored. Perfect for private scratchpads. |
| **2. One-off Test Scripts** | `<target-directory>/scratch/` or `.gemini/scratch/` | ❌ Gitignored. Safe to run without pollution. |
| **3. Project Documentation** | `docs/dev-guide/`, `docs/user-guide/`, `docs/ecosystem/` | ✅ Tracked. Write clear markdown for humans/agents. |
| **4. Architectural Blueprints** | `docs/vision/` | ✅ Tracked. Used for system design planning. |
| **5. AI Rules & Governance** | `.agents/rules/` | ✅ Tracked. Immutable laws all agents must follow. |
| **6. AI Workflows (SOPs)** | `.agents/workflows/` | ✅ Tracked. Operational procedures for tasks. |
| **7. Source Code & Logic** | `src/` (TypeScript ESM) | ✅ Tracked. Core application code. |
...
3. Move the file:
   ...
   ```
4. Stage the intended files and re-commit!
