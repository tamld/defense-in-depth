...
async function makeScraperRepo(prefix) {
  ...
  git(
    root,
    ["commit", "-q", "--no-gpg-sign", "-m", "fix(core): patch a"],
...
test("scanHistory drops FN when reverted commit is absent from the scanned window", async () => {
  ...
  try {
    ...
    run(
      ["commit", "--allow-empty", "-q", "--no-gpg-sign", "-m", "chore: base"],
      Object.fromEntries([stamp, ["GIT_COMMITTER_DATE", stamp[1]]]),
    );
    ...
    run(
      [
        ...
        "revert: undo patch",
        "-m",
        "This reverts commit deadbeef00112233445566778899aabbccddeeff.",
      ],
