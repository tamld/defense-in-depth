...
test("verify without staged files or --files short-circuits politely", async () => {
...
test("verify reports a clean pass with summary line", async () => {
...
test("verify blocks on hollow artifacts and exits 1", async () => {
...
test("hook mode stays silent even without CI suppression", async () => {
...
test("DSPy-forced config degrades with banner instead of failing", async () => {
  ...
  try {
    ...
    assert.equal(res.exitCode, null, `err: ${res.err}`);
    assert.ok(res.err.includes("DSPy unavailable"), `stderr: ${res.err}`);
  } finally {
    await rm(root, { recursive: true, force: true });
  }
});
