...
import { emitOneHint, emitAllHints, formatHint, isChannelEnabled } from "../dist/cli/hints-emit.js";
  ...
  try {
    ...
    await withEnv({ CI: "false", NO_HINTS: undefined }, async () => {
      ...
      assert.ok(
        ...
        "cooldownDays:0 must allow immediate re-emission of same hint",
      );
    });
  } finally {
...
test("compound suppression guards cover multi-variable environments", async (t) => {
  await t.test(
    "CI=true + NO_HINTS=1 + channel disabled -> emitOneHint returns null, no stderr write",
    async () => {
  ...
  await t.test(
    "CI=true + NO_HINTS=1 + channel enabled + eligible hint -> emitOneHint returns null (CI trumps)",
    async () => {
      const { execFileSync } = await import("node:child_process");
  ...
  await t.test(
    "NO_HINTS=1 + CI=false + NO_COLOR=1 + TTY=true -> formatHint output contains NO ANSI (NO_COLOR trumps)",
    async () => {
      const savedIsTTY = process.stderr.isTTY;
      process.stderr.isTTY = true;
      try {
        await withEnv({ NO_HINTS: "1", CI: "false", NO_COLOR: "1" }, async () => {
          ...
          assert.ok(
            !text.includes("\x1b["),
            "no ANSI color codes when NO_COLOR=1 even if TTY=true",
          );
