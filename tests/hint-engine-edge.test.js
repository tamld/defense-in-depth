...
async function writeFeedbackTp(root) {
  ...
  console.error = () => {};
  try {
    await handleFeedbackCommand(root, [
      "tp",
      "--guard",
      "hollow-artifact",
      "--finding",
...
test("hint rules branch on repo shape", async (t) => {
  await t.test("H001 fires on dspy-less repo with >=5 commits", async () => {
    ...
    try {
      const eligible = idsOf(
        await evaluateHints({ projectRoot: root, state: loadHintState(root) }),
      );
  ...
  await t.test("cooldown boundary: shown hint excluded at 30d, re-included at 0d", async () => {
    ...
    try {
      ...
      const cooled = idsOf(
        await evaluateHints({ projectRoot: root, state: loadHintState(root), cooldownDays: 30 }),
      );
      assert.ok(!cooled.includes("H-001-no-dspy"), "within cooldown must be excluded");
      const reset = idsOf(
        await evaluateHints({ projectRoot: root, state: loadHintState(root), cooldownDays: 0 }),
      );
