...
test("Hollow Artifact Guard (v0.5)", async (t) => {
  await t.test("deterministic checks", async (_st) => {
  ...
  await t.test("DSPy logic correctly filters non-semantic files", async (_st) => {
    const { dir, cleanup } = createTempWorkspace();

    fs.writeFileSync(
      path.join(dir, "binary.exe"),
      "just pretending to be a binary that happens to have the exact length needed so it passes deterministic checks: this is a completely valid string meant to be over 50 characters to bypass the deterministic hollow artifact check.",
    );
  ...
  await t.test("DSPy graceful degradation (no semantic data)", async (_st) => {
    const { dir, cleanup } = createTempWorkspace();
    fs.writeFileSync(
      path.join(dir, "good.md"),
      "This is a totally valid document with plenty of words to pass the minimum length check, unlike an empty file.",
    );
  ...
  await t.test("DSPy warning emitted when score < 0.5", async (_st) => {
    const { dir, cleanup } = createTempWorkspace();
    fs.writeFileSync(
      path.join(dir, "sneaky.md"),
      "This is a document that technically passes simple string checks by being very long but completely lacks actual semantic meaning because it is just generated filler text designed to bypass simple text string matching algorithms.",
    );
  ...
  await t.test("DSPy boundary: score exactly 0.5 should NOT warn", async (_st) => {
    const { dir, cleanup } = createTempWorkspace();
    fs.writeFileSync(
      path.join(dir, "borderline.md"),
      "This document is on the boundary of quality. It is exactly at the threshold and should not trigger a warning because the condition is strictly less than 0.5.",
    );
  ...
  await t.test("DSPy boundary: score 0.0 should warn", async (_st) => {
    const { dir, cleanup } = createTempWorkspace();
    fs.writeFileSync(
      path.join(dir, "zero.md"),
      "This document will receive the absolute worst score from the evaluator, testing the lower boundary of the scoring system to ensure it triggers a warning.",
    );
