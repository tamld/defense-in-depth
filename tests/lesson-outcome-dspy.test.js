...
import { scanOutcomes, appendRecall } from "../dist/core/lesson-outcome.js";
...
async function makeScanRoot(prefix) {
  ...
  git(root, ["commit", "-q", "--no-gpg-sign", "-m", "chore: seed scan repo"], {
...
test("scanOutcomes DSPy fuzzy-match branches", async (t) => {
  await t.test("unreachable endpoint degrades once with single warning", async () => {
    ...
    try {
      ...
      const warns =
        chunks.join("").split("[scan-outcomes] DSPy fuzzy match unavailable").length - 1;
