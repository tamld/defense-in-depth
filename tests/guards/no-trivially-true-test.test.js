...
test("noTriviallyTrueTestGuard — contract shape & disabled state", async (t) => {
...
test("noTriviallyTrueTestGuard — blocks trivial assertions & empty test bodies", async (t) => {
  await t.test("blocks expect(true).toBe(true) and assert.strictEqual(1, 1)", async () => {
    const root = await makeTmpDir();
    try {
      await mkdir(path.join(root, "tests"), { recursive: true });
      await writeFile(
  ...
  await t.test("blocks test block executing code with zero assertions", async () => {
    const root = await makeTmpDir();
    try {
      await mkdir(path.join(root, "tests"), { recursive: true });
      await writeFile(
...
test("noTriviallyTrueTestGuard — allows real assertions & allowlists", async (t) => {
  await t.test("allows genuine assertion and assert.throws", async () => {
    const root = await makeTmpDir();
    try {
      await mkdir(path.join(root, "tests"), { recursive: true });
      await writeFile(
  ...
  await t.test("skips fixtures directory, missing files, and non-test files", async () => {
    const root = await makeTmpDir();
    try {
