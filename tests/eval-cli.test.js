...
test("eval CLI handler branches", async (t) => {
  ...
  await t.test("clean artifact passes and leaves exit code untouched", async () => {
    ...
    try {
      ...
      assert.ok(
        `${res.stderrRaw}\n${res.errors.join("\n")}`.includes("DSPy unavailable"),
        "graceful degradation banner expected",
      );
    } finally {
      await rm(root, { recursive: true, force: true });
    }
...
test("eval extension-injection and endpoint override arms", async (t) => {
  await t.test("extensionless target skips extension push but still evaluates", async () => {
    ...
    try {
      ...
      await writeFile(
        p,
        "A plain artifact with more than fifty characters of substantive content for the guard.",
        "utf8",
      );
  ...
  await t.test("unusual extension is injected into guard extensions", async () => {
    ...
    try {
      ...
      await writeFile(
        p,
        "TypeScript-flavored notes file with plenty of substantive content beyond fifty chars.",
        "utf8",
      );
      const res = await runEval(root, [p]);
  ...
  await t.test("custom dspyEndpoint override degrades through closed port", async () => {
    ...
    try {
      ...
      await writeFile(
        p,
        "Substantive documentation body long enough to satisfy the meaningful-content heuristic.",
        "utf8",
      );
      const res = await runEval(root, [p]);
      ...
      assert.ok(
        signal.includes("DSPy unavailable"),
        "override endpoint must be attempted then degrade",
      );
    } finally {
      await rm(root, { recursive: true, force: true });
    }
